...
class GeneticAlgorithm:
    def __init__(self, chromatic_num):
        ...
        self.num_vertices = 0
        self.adjacency_list = {}
        self.descent_quality_boundary = 5
        self.max_descent_iters = 3
    ...
    def descent(self, chromosome):
        for k in range(self.max_descent_iters):
            current_chromosome = chromosome.code
            for i in chromosome.code:
                for j in self.adjacency_list[i]:
                    if chromosome.code[i] == chromosome.code[j]:
                        available_colors = self.get_available_colors(chromosome, i)
                        if len(available_colors) > 0:
                            chromosome[i] = random.choice(available_colors)
            if chromosome.fitness > self.calculate_fitness(chromosome):
                chromosome = Chromosome(chromosome, self.calculate_fitness(chromosome))
    ...
    def create_generation(self, reproduction_group):
        ...
        # Local search
        for i in range(self.population_size):
            if current_generation[i].fitness < self.descent_quality_boundary:
                current_generation[i] = self.descent(current_generation[i])
